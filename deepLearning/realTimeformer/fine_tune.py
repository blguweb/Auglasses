...
def fine_tune(args, file_name, file_path, nw, device):
    path_label = '{}_ft{}_in{}_{}_{}_tdo{}_cdo{}_im{}_au{}_ci{}_co{}_lr{}_pl{}_s{}_re{}_dm{}_nh{}_el{}_dl{}_dff{}_{}_{}_{}_{}.pkl'.format(
        args.time_run,
        args.is_fine_tune,
        args.is_init,
    ...
    for epoch in range(args.train_epochs):
        ...
        for i, (batch_x, batch_y) in enumerate(vali_loader): # [batch, au_len,labels/imu_len,channel]
            if i >= int(len(vali_loader) / 2):
                continue
            iter_count += 1
            count_ += 1
        ...
        for file_name in os.listdir(opt.dataset_path):
            if file_name.startswith(start_name) and file_name.endswith(str(opt.imu_len)):
                ...
                # 构造完整的文件路径
                    ...
                    print(file_name)
                    if opt.is_fine_tune:
                        fine_tune(opt, file_name, file_path, nw, device)
                    else:
                        # 在已有模型基础上测试另外一个人的数据
...
if __name__ == '__main__':
    ...
    parser.add_argument('--turns_num', type=int, default=1)
