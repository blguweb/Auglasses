...
def recover_imu_signal(imu_dataset, sample_overlap=5):
    # 初始化恢复后的信号列表
    recovered_signal = copy.deepcopy(imu_dataset[0,:,:])
    np.set_printoptions(threshold=1000)
    # 遍历窗口数据集，除了最后一个窗口
    for i in range(1, len(imu_dataset)):
        next_window = imu_dataset[i]

        # 获取下一个窗口的前几个数值 用第一维度
        next_start_segment = next_window[:sample_overlap, 0]
        # print(next_window)
        

        # 在当前窗口中寻找这些数值连续出现的位置
        overlap_index = -1  # 默认值，表示未找到重叠
        for j in range(len(recovered_signal) - sample_overlap + 1):
            if np.array_equal(recovered_signal[j:j+sample_overlap, 0], next_start_segment):
                overlap_index = j
                break

        # 添加当前窗口到重叠开始之前的数据
        if overlap_index >= 0:
            recovered_signal = np.vstack((recovered_signal[:overlap_index], next_window))
        else:
            print("error")
...
def is_state(signal,acc_threshold=0.1,gyro_threshold=0.5):
    # batchsize; len, 200, 12):
    # batchsize = 1
    signal = signal.squeeze(0).cpu().numpy()
    signal_serial = recover_imu_signal(signal, sample_overlap=5)
    # 计算每列的方差
    # print(np.max(signal_serial, axis=0))
    means = np.mean(signal_serial, axis=0)
    if np.all(means[0:3] < acc_threshold) and np.all(means[3:6] < gyro_threshold) and np.all(means[6:9] < acc_threshold) and np.all(means[9:12] < gyro_threshold):
        print("normal")
        return True
    else:
        return False
...
def model_test(opt, imu_data, au_data, device, criterion, model):
    ...
    test_path = os.path.join('./longTimeResultZeros/', opt.time_run, opt.downstream_dataset, opt.model_name, str(opt.total_length))
    ...
    with torch.no_grad():
        for i, (batch_x, batch_y) in enumerate(test_loader):
            ...
            history_len = opt.au_len - opt.pred_len
            # base
            base_state = torch.zeros_like(batch_y[:, -history_len:, :]).float()
            base_state[:,:,5] = 1
            base_state = base_state.to(device)
            ...
            for times in range(0, batch_y.shape[1],opt.total_length):
                ...
                for se_i in range(0, opt.total_length, history_len):
                    ...
                    if is_state(intput_x[:,0:history_len,:,:]):
                        result[:, se_i-times:(se_i + history_len -times),:] = base_state
                    
                    # intput_y = batch_y[:,se_i:se_i+opt.au_len,:].float().to(device)

                    dec_inp = torch.zeros_like(result[:, -opt.pred_len:, :]).float()
                    dec_inp = torch.cat([result[:, se_i-times:(se_i + history_len -times), :], dec_inp], dim=1).float().to(device)
                    ...
                    if se_i == 0 + times:
                        ...
                        result = torch.cat([result[:,:(se_i- times + history_len),:], output_former[:,-opt.pred_len:, :]], dim=1) # 只用预测部分
...
def imuInference(opt, imu, au, device, window_length):
    ...
    pd, gt_au = model_test(opt, imu_dataset, au_gt, device, criterion, model) # [expression_len, au_channels]
    # save
    # save_list = {"gt":gt_au,
    #              "pd":inference_au,
    #              "index":index_pred}
...
if __name__ == '__main__':
    ...
    parser.add_argument('--time_run',type=str,default='04191602')
    parser.add_argument('--num_worker', type=int, default=1)
    parser.add_argument('--cuda',type=str,default='cuda:0')
    parser.add_argument('--use_cuda', default=True)
    parser.add_argument('--batch_size', type=int, default=1)
    parser.add_argument('--total_length', type=int, default=5400)
    parser.add_argument('--model_name', type=str, default='04191602_pred45_0.5_1.5_tdo0.1_cdo0.5_im200_au60_ci12_co14_lr0.01_pl20_s4_re0_dm256_nh8_el6_dl6_dff512_within_cut_32_auex.pkl')
    ...
    parser.add_argument('--downstream_dataset', type=str, default='imu_au_wzf1.csv', help='pred au len of decoder')
