...
def general_dataset(user_mode, dataset_type, total_people, spilt_mode='cut', data_root='./data/',
    ...
    if user_mode == 'within':
        ...
        for person, files in data_files.items():
            ...
            print(person_dataset_path, len(dataset_y_train), len(dataset_y_test), len(dataset_y_val))
            ...
            for dataset_name, dataset_array in [('train_data', dataset_x_train), 
                                                ('train_label', dataset_y_train), 
                                                ('test_data', dataset_x_test), 
                                                ('test_label', dataset_y_test),
                                                ('val_data', dataset_x_val),
                                                ('val_label', dataset_y_val)]:
            ...
            # 存储长度
            txn.put( f'train_len'.encode('utf-8'), pickle.dumps(len(dataset_x_train), protocol=4))
            txn.put( f'test_len'.encode('utf-8'), pickle.dumps(len(dataset_x_test), protocol=4))
            txn.put( f'val_len'.encode('utf-8'), pickle.dumps(len(dataset_x_val), protocol=4))
        ...
        for test_person in data_files.keys():
            ...
            # np.save(person_dataset_path, xy_dict)
            print(person_dataset_path, len(train_data), len(test_data),len(val_data))
            # with open(person_dataset_path, 'wb') as f:
            #     pickle.dump(xy_dict, f, protocol=4)
            with lmdb_holder.begin(write=True) as txn:
                ...
                for dataset_name, dataset_array in [('train_data', train_data), 
                    ...
                    for index, value in enumerate(dataset_array):
                        ...
                        txn.put(key, value_serialized)
                # 存储长度
                txn.put( f'train_len'.encode('utf-8'), pickle.dumps(len(train_data), protocol=4))
                txn.put( f'test_len'.encode('utf-8'), pickle.dumps(len(test_data), protocol=4))
                txn.put( f'val_len'.encode('utf-8'), pickle.dumps(len(val_data), protocol=4))
                txn.commit()
            lmdb_holder.close()  # 关闭LMDB环境
