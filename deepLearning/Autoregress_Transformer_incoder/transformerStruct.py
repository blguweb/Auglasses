...
class Encoder(nn.Module):
    def __init__(self,in_channel,sq_len, dropout, n_layers,d_model,n_heads,d_ff,inference):
    ...
    def forward(self, enc_inputs):
        if self.inference == True:
            pos_emd = self.pos_emb[:enc_inputs.shape[1],:]
        else:
            pos_emd = self.pos_emb
        # print(enc_inputs.shape)
        # print(self.val_emb(enc_inputs).shape)
        # print(pos_emd.shape)
        enc_outputs = self.dropout(self.val_emb(enc_inputs) + pos_emd)
        device = enc_inputs.device
        # dec_self_attn_pad_mask = get_attn_pad_mask(dec_inputs, dec_inputs)
        enc_self_attn_subsequent_mask = get_attn_subsequent_mask(enc_inputs).to(device)
        enc_self_attn_mask = torch.gt(enc_self_attn_subsequent_mask, 0)
