...
def slidingWindow(imu_path, au_path, window_length, step_length, desired_length=1200):
    ...
    for index in range(30,au_data.shape[0]-60):
        ...
        dec_in_lists.append(np.array(au_data[index-1,1:]))
    ...
    for slid_index in range(0, len(au_lists), step_length):
        ...
        current_dec_window = dec_in_lists[slid_index:end_index]


        X.append(np.array(current_imu_units))
        Y.append(np.array(current_au_window))
        A.append(np.array(current_dec_window))
...
# 
            if person != 'lls':
            # ['lyr','cyx' ,'dll', 'dyw', 'hyj', 'lls']: wzf oyh
                continue
            ...
            for imu_ex_file in imu_ex_files:
                ...
                if au_ex_file in au_ex_files:
                    # read
                    personal_x, personal_y, dec_in = slidingWindow(os.path.join(data_root, imu_ex_file), os.path.join(data_root, au_ex_file),
                    ...
                    for px_i in range(border1s[0],border2s[0]):
                        ...
                        train_dec_key = f'train_dec_{train_length}'.encode('utf-8')
                        ...
                        txn.put(train_dec_key, pickle.dumps(dec_in[px_i],protocol=4))
                        train_length += 1
                        txn.commit()
                    ...
                    for px_i in range(border1s[1],border2s[1]):
                        ...
                        val_dec_key = f'val_dec_{val_length}'.encode('utf-8')
                        ...
                        txn.put(val_dec_key, pickle.dumps(dec_in[px_i],protocol=4))
                        val_length += 1
                        txn.commit()
                    ...
                    for px_i in range(border1s[2],border2s[2]):
                        ...
                        test_dec_key = f'test_dec_{test_length}'.encode('utf-8')
                        ...
                        txn.put(test_dec_key, pickle.dumps(dec_in[px_i],protocol=4))
                        test_length += 1
                        txn.commit()
            ...
            for imu_au_file in imu_au_files:
                ...
                if au_au_file in au_au_files:
                    personal_x, personal_y, dec_in = slidingWindow(os.path.join(data_root, imu_au_file), os.path.join(data_root, au_au_file),
                                                           window_length=window_length,step_length=step_length,
                                                           desired_length=desired_length)
                    dataset_length = len(personal_y)
                    num_train = int(dataset_length * 0.7)
                    ...
                    for imu_ex_file in imu_ex_files:
                        ...
                        if au_ex_file in au_ex_files:
                            ...
                            for vx_i in range(num_train, dataset_length):
                                ...
                                txn.put(val_dec_key, pickle.dumps(dec_in[vx_i],protocol=4))
                                val_length += 1
                                txn.commit()
...
if __name__ == '__main__':
    general_dataset(user_mode='within',
                    ...
                    window_length=60, # au
                    step_length=1,
                    data_root='../new_data_P14_auex/',
                    dataset_root='./withindataset/',
