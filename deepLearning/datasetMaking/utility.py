...
'''
    @description:  归一化
'''
def normalization(signal, acc_max = 1.2, gyro_max = 80):
    print("max",acc_max,gyro_max)
    acc_min = -acc_max
    gyto_min = -gyro_max
    # acc
    ...
    # gyto
    signal[:,9:12] = 2 * ((signal[:,9:12] - gyto_min) / (gyro_max - gyto_min)) - 1
    signal[:,3:6] = 2 * ((signal[:,3:6] - gyto_min) / (gyro_max - gyto_min)) - 1
    return signal
